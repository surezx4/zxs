...
normalize_raster <- function(ras) {
  # 获取数据范围
  min_val <- minValue(ras)
  max_val <- maxValue(ras)
  ...
  # 线性拉伸到0-1范围
  normalized <- (ras - min_val) / (max_val - min_val)
...
repair_band_with_normalization <- function(input_file, ref_file) {
  start_time <- Sys.time()
  ...
  tryCatch({
    ...
    # 记录原始数据范围
    orig_min <- minValue(band_ras)
    orig_max <- maxValue(band_ras)
    log_message(paste("原始数据范围:", orig_min, "-", orig_max))
    ...
    # 记录修复后数据范围
    rep_min <- minValue(output_ras)
    rep_max <- maxValue(output_ras)
    log_message(paste("修复后数据范围:", rep_min, "-", rep_max))
...
for (i in seq_along(band_files)) {
  ...
  # 修复波段
  repaired <- repair_band_with_normalization(band, ref_band)
...
if (success_count > 0) {
  ...
  if (nlayers(band_stack) >= 3) {
    # 创建规范化副本用于显示
    normalized_stack <- stack(lapply(1:nlayers(band_stack), function(i) {
      normalize_raster(band_stack[[i]])
    }))
    
    # 应用直方图均衡化拉伸
    plotRGB(normalized_stack, r = 3, g = 2, b = 1, 
            stretch = "hist", 
            main = "修复后的Landsat图像 (直方图均衡化)",
            colNA = "black")  # 设置NA值为黑色
  } else if (nlayers(band_stack) > 0) {
    # 单波段显示
    normalized_band <- normalize_raster(band_stack[[1]])
    plot(normalized_band, 
         main = paste("修复后的Landsat波段", names(band_stack)[1]),
         col = gray.colors(256))
  }
